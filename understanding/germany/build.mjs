...
function render(source) {
  ...
  const converted = source.replace(/\{%\s*(\w+)(?:\s+style="([^"]+)")?\s*%\}/g, (_, tag, style) => {
    if (tag.startsWith('end')) {
      assert.equal(stack.pop(), tag.slice(3), `Unbalanced hint/stepper tag: ${tag}`);
      return '\n\n</div>\n\n';
    }
    assert(['hint', 'stepper', 'step'].includes(tag), `Unsupported tag: ${tag}`);
    stack.push(tag);
    return `\n\n<div class="${tag}${style ? ` ${escape(style)}` : ''}">\n\n`;
  });
  assert.equal(stack.length, 0, 'Unclosed hint/stepper tags');
...
for (const [file, content] of outputs) {
  assert(!content.includes('{%'), `Unconverted hint/stepper markup in ${file}`);
...
function walk(value) {
  ...
  if (value.directory) {
    assert(existsSync(resolve(root, '..', value.directory)), `Missing content directory: ${value.directory}`);
...
for (const folder of ['markdown', 'chinese/markdown']) {
  ...
  for (const file of files) {
    ...
    render(source); // Validate hint/stepper tag balance in both editions.
...
console.log(`${check ? 'Verified' : 'Built'} ${outputs.size} files; ${chapters.length} chapters, local links and content directories checked.`);
